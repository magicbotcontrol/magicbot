...
$cmdPath = Join-Path $PSScriptRoot 'ZENQUANT-WATCHDOG.cmd'
$iconPath = Join-Path $projectRoot 'deployment\windows-worker\zenquant-worker.ico'
$shortcutName = 'ZENQUANT WATCHDOG.lnk'
$shortcutPath = Join-Path $desktopPath $shortcutName
$legacyShortcutPaths = @(
  (Join-Path $desktopPath 'ZENQUANT-WATCHDOG.lnk'),
  (Join-Path $desktopPath 'ZENQUANT WATCHDOG ON.lnk')
)

if (-not (Test-Path $cmdPath)) {
  throw "Arquivo do launcher watchdog nao encontrado: $cmdPath"
}
...
"@
  ...
  $graphics.DrawString('WATCH', $smallFont, $textBrush, (New-Object System.Drawing.RectangleF(0, 148, 236, 36)), $stringFormat)
  $graphics.DrawString('DOG', $smallFont, $textBrush, (New-Object System.Drawing.RectangleF(0, 182, 236, 32)), $stringFormat)
...
$shortcut.Description = 'Liga a stack ZenQuant, envia reconexao e inicia o watchdog automatico.'
$shortcut.Save()

Write-Host "Atalho criado em: $shortcutPath"
Write-Host "Launcher do watchdog: $cmdPath"
