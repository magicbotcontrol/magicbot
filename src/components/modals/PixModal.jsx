export function PixModal({ showPixModal, pixAmount, handlePixSuccess, setShowPixModal, t, formatMoney }) {
  if (!showPixModal) {
    return null;
  }
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white dark:bg-[#1E293B] rounded-3xl w-full max-w-xs p-6 space-y-4 shadow-2xl text-center">
        ...
        <p className="text-xs text-gray-500">
          {t.amountToPay}: <span className="font-bold text-orange-600">{formatMoney(pixAmount, 'BRL')}</span>
        </p>
