export function PremiumBlockedTab({ t, setActiveTab, title, subtitle, ctaLabel }) {
  return (
    <div className="mx-auto max-w-3xl animate-fade-in">
      <div className="rounded-3xl border border-red-200 bg-white p-8 text-center shadow-sm dark:border-red-900/60 dark:bg-[#0B1220]">
        ...
        <h2 className="mt-5 text-xl font-black text-gray-900 dark:text-white">{title || t.premiumBlockedTitle}</h2>
        <p className="mt-3 text-sm text-gray-500 dark:text-[#94A3B8]">{subtitle || t.premiumBlockedSubtitle}</p>
        <button
          type="button"
          onClick={() => setActiveTab('shop')}
          className="mt-6 rounded-2xl bg-[#FF6B00] px-5 py-3 text-sm font-bold text-white transition-colors hover:bg-[#FF7F1F]"
        >
          {ctaLabel || t.premiumBlockedCta}
        </button>
      </div>
    </div>
  );
